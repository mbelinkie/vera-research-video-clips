...
type SelectionCommandPanelProps = Readonly<{
  ...
  selectionExportBlockReason: string | undefined;
  attestedNoSpeech: boolean;
...
export function SelectionCommandPanel(props: SelectionCommandPanelProps) {
  const {
    ...
    selectionHasTranscriptEvidence,
  ...
  return (
    <>
      ...
      <details className="export-settings-panel">
        ...
        <label className="export-checkbox">
          <input
            ...
            disabled={exportOnlySettingsState !== "ready" || attestedNoSpeech}
            onChange={(event) => {
              setOverrideFields((current) =>
        ...
        {attestedNoSpeech ? (
          <p className="muted">
            Attested no-speech exports use verified empty sidecars and cannot
            embed a subtitle stream.
          </p>
      ...
      <div className="selection-actions">
        <button
          ...
          disabled={
            ...
            !user ||
            !selectedVideoSnapshot ||
            (selectionHasTranscriptEvidence && !languageEvidenceReady) ||
            offlineCachedWorkspace
          }
          onClick={() => void queueClipOnly()}
        >
          {loggedClipId ? "Logged" : "Log clip"}
        </button>
        <details className="split-action-menu">
          <summary aria-label="More clip actions">More actions</summary>
          <div className="split-action-menu-items" role="menu">
            <button
              type="button"
              role="menuitem"
              disabled={
                ...
                (selectionHasTranscriptEvidence && !languageEvidenceReady) ||
                offlineCachedWorkspace ||
                Boolean(selectionExportBlockReason) ||
                !sourceRightsConfirmed ||
                loggedSettingsState !== "ready"
              }
              onClick={() => void requestLoggedExport()}
            >
              {loggedExportRequestId ? "Export queued" : "Log and export"}
            </button>
            <button
              type="button"
              role="menuitem"
      ...
      <span className="selection-action-help">
        Log clip starts no media work. Export without logging creates no project
        research record.
      </span>
      {selectionExportBlockReason ? (
        <p className="form-message error">{selectionExportBlockReason}</p>
      ) : null}
      <p className="form-message" role="status">
        {clipActionMessage ??
          (!projectId
