...
- Local migration suite: `25` tests passed. Focused cloud migration suite: `2`
  tests passed. Migration CLIs reported `37` cloud and `31` local migrations
  valid on empty databases.
...
- The broad Vitest run reached `606` passed and `4` skipped. Ten migration-list
  ...
  this implementation.
- `npm run typecheck` reaches only the inherited concurrent PUNCH snapshot error
  `packages/catalog/src/index.ts: mapClipComment is not defined`; no
  platform-search type error remains.
...
- Merge/rebase this commit after the concurrent PUNCH shell/comment lane is
  complete, then rerun the full typecheck/Vitest suite and resolve final shell
  integration conflicts in the PUNCH-owned lane.
